...
function init() {
  ...
  // build mock-api
  exec('ng build router-testing', (error, stdout) => {
    if (error) {
      console.log(`stdout: ${error.message}`)
      return;
    }
    console.log(`stdout: ${stdout}`);
    process.chdir('dist/router-testing');
    ...
    exec('npm link', (error, stdout) => {
      ...
      exec('npm link @yunzhi/ng-router-testing', (error, stdout) => {
        if (error) {
...
watch(['projects/router-testing/src'], {recursive: true}, () => {
  init();
});
