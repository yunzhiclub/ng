...
export class ApiPrefixAndMergeMapInterceptor implements HttpInterceptor {
  ...
  public static _api = '/api' as string;

  static forRoot(data: {
    api: string
  }): typeof ApiPrefixAndMergeMapInterceptor {
    ApiPrefixAndMergeMapInterceptor.setApi(data.api);
  ...
  public static getApiUrl(url: string): string {
    ...
    if (url.startsWith('/')) {
      return ApiPrefixAndMergeMapInterceptor._api + url;
  ...
  public static setApi(api: string) {
    ApiPrefixAndMergeMapInterceptor._api = api;
  }
